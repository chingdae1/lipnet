...
class Decoder:
    def __init__(self, labels, lm_path=None, alpha=1, beta=1.5, cutoff_top_n=40, cutoff_prob=0.99,
                 beam_width=200, num_processes=24, blank_id=0):
        self.vocab_list = ['_'] + labels  # NOTE: blank symbol
        self._decoder = CTCBeamDecoder(['_@'] + labels[1:], lm_path, alpha, beta, cutoff_top_n,
    ...
    def decode_beam(self, logits, seq_lens):
        ...
        for i in range(tlogits.size(0)):
            output_str = ''.join(
                map(lambda x: self.vocab_list[x], beam_result[i][0][:out_seq_len[i][0]]))
